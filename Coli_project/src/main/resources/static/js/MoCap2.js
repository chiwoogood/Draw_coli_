  ...
  if (file && file.type.startsWith('image/')) { // 이미지 파일인지 확인
      ...
      reader.onload = function(e) {
         ...
         canvasCtx.save();
              // 캔버스를 X축 방향으로 반전
              canvasCtx.scale(-1, 1);
              // 캔버스를 오른쪽으로 옮겨서 이미지가 캔버스 바깥으로 나가지 않도록 조정
            ...
            console.log("이미지 로딩 완료");
              // 이미지가 로드된 후 Holistic 모델에 이미지 보내기
              detectionFrame();
          };
      };
...
import { DRACOLoader } from 'https://unpkg.com/three@0.133.1/examples/jsm/loaders/DRACOLoader.js';

// DRACOLoader 설정
//const dracoLoader = new DRACOLoader();
//dracoLoader.setDecoderPath('https://unpkg.com/three/examples/js/libs/draco/'); // DRACO 디코더의 경로 설정


// GLTFLoader에 DRACOLoader 추가
...
let render_w = 600; // 너비를 600으로 설정
const render_h = 725; // 높이를 450으로 설정
canvasElement.width = render_w;
canvasElement.height = render_h;
const workHistoryDiv = document.getElementById('my-work-history');
renderer.setSize(render_w, render_h);
renderer.setViewport(0, 0, render_w, render_h);
renderer.shadowMap.enabled = true;
workHistoryDiv.appendChild(renderer.domElement); // <div> 내부에 <canvas> 추가
...
camera_ar.position.set(0, 1, 3); // Y축은 높이를 조정하고, Z축은 모델과의 거리를 조정합니다.
...
hemiLight.intensity = 1.5; // 기본값보다 높게 설정
scene.add(hemiLight);

const dirLight = new THREE.DirectionalLight(0xffffff); //AmbientLight, DirectionalLight
dirLight.position.set(3, 10, 10);
// dirLight.intensity = 1.5; // 기본값보다 높게 설정
...
loader.load(cpath + '/pose/Xbot.glb', function (gltf) {
  model = gltf.scene;
  model.scale.x = -1; // X축을 기준으로 모델 뒤집기
  scene.add(model);
  let bones = [];
  ...
  skeleton_helper.visible = false; //true 였던걸 false로 수정함
...
async function detectionFrame() {
    try {
      ...
      console.error('Holistic 처리 중 오류 발생:', error);
  }
 ...
 $(function(){
        $(".btn_download").click(function(e){
            // Three.js를 사용하여 생성된 canvas를 선택
            var canvas = renderer.domElement; // 여기서 renderer는 Three.js에서 사용하는 렌더러입니다.
